  ...
  function formRow({ form, setForm, onSave, onCancel, isNew }) {
    return (
      <div className={`px-4 py-3 border-b ${isNew ? 'bg-blue-50/40' : 'bg-yellow-50/40'}`}>
        {/* 手機單欄、sm 兩欄、lg 才回到原本的 5 欄 inline 版型 */}
        <div className="grid gap-2 mb-2 grid-cols-1 sm:grid-cols-2 lg:grid-cols-[1fr_72px_110px_110px_1fr]">
          <div className="min-w-0"><label className="block text-xs text-gray-500 mb-0.5">賽事名稱 *</label>
          ...
          <div className="min-w-0"><label className="block text-xs text-gray-500 mb-0.5">代碼</label>
            <input value={form.code} onChange={e => setForm(f => ({ ...f, code: e.target.value }))} placeholder="HBL"
              className="w-full border border-gray-300 rounded-lg px-2.5 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400" />
          </div>
          <div className="min-w-0"><label className="block text-xs text-gray-500 mb-0.5">開始日期</label>
          ...
          <div className="min-w-0"><label className="block text-xs text-gray-500 mb-0.5">結束日期</label>
            <input type="date" value={form.endDate} onChange={e => setForm(f => ({ ...f, endDate: e.target.value }))}
              className="w-full border border-gray-300 rounded-lg px-2 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400" />
          </div>
          <div className="min-w-0 sm:col-span-2 lg:col-span-1"><label className="block text-xs text-gray-500 mb-0.5">備註</label>
            <input value={form.note} onChange={e => setForm(f => ({ ...f, note: e.target.value }))} placeholder="例：準決賽 前8強"
              className="w-full border border-gray-300 rounded-lg px-2.5 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400" />
          </div>
        </div>
        <div className="flex justify-end gap-2">
          <button onClick={onCancel} className="px-3 py-2 min-h-[40px] text-sm text-gray-500 hover:bg-gray-200 rounded-lg">取消</button>
          <button onClick={onSave} disabled={saving || !form.name}
            className="px-4 py-2 min-h-[40px] text-sm bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-40 font-medium">
    ...
    <div className="flex flex-col h-full min-w-0">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between px-4 sm:px-6 py-3 sm:py-4 border-b bg-white shrink-0">
        <div className="min-w-0">
          <h1 className="text-lg sm:text-xl font-bold text-gray-800">高中體總贊助管理</h1>
          <p className="text-sm text-gray-500 break-words">中華民國高中體育總會 · 合約年度：民國 113–115 年（2024–2026）</p>
        </div>
        <a href="https://www.hhsaa.org.tw" target="_blank" rel="noopener noreferrer"
          className="text-sm text-blue-500 hover:text-blue-700 px-3 py-2 min-h-[44px] inline-flex items-center rounded-lg border border-blue-200 hover:bg-blue-50 transition-colors self-start shrink-0">
          高中體總官網 ↗
        </a>
      </div>

      {/* ── Summary cards：手機 2 欄、平板 3 欄、桌面維持 5 欄 ── */}
      <div className="px-4 sm:px-6 py-3 grid grid-cols-2 md:grid-cols-3 xl:grid-cols-5 gap-3 border-b bg-gray-50/60 shrink-0">
        <div className="bg-white rounded-xl border border-gray-200 px-3 sm:px-4 py-3 min-w-0">
          <p className="text-xs text-gray-500">合約總金額</p>
          <p className="text-lg sm:text-xl font-bold text-gray-800 mt-0.5">1,110 <span className="text-sm font-normal text-gray-500">萬</span></p>
          <p className="text-xs text-gray-500">3 年 · 9 期</p>
        </div>
        <div className="bg-white rounded-xl border border-gray-200 px-3 sm:px-4 py-3 min-w-0">
          <p className="text-xs text-gray-500">已付款</p>
          <p className="text-lg sm:text-xl font-bold text-emerald-600 mt-0.5">{paidAmount} <span className="text-sm font-normal text-gray-500">萬</span></p>
          <p className="text-xs text-gray-500">{paidList.length}/9 期</p>
        </div>
        <div className="bg-white rounded-xl border border-gray-200 px-3 sm:px-4 py-3 min-w-0">
          <p className="text-xs text-gray-500">款項逾期</p>
          <p className={`text-lg sm:text-xl font-bold mt-0.5 ${overdueList.length > 0 ? 'text-red-500' : 'text-gray-500'}`}>
            {overdueList.length} <span className="text-sm font-normal text-gray-500">筆</span>
          </p>
          <p className="text-xs text-gray-500 break-words">{overdueList.length > 0 ? `${overdueList.reduce((s,p)=>s+p.amount,0)} 萬元` : '全部正常 ✓'}</p>
        </div>
        <div className="bg-white rounded-xl border border-gray-200 px-3 sm:px-4 py-3 min-w-0">
          <p className="text-xs text-gray-500">廣告素材交稿</p>
          <p className="text-lg sm:text-xl font-bold text-blue-600 mt-0.5">{submittedCount}<span className="text-sm font-normal text-gray-500">/{AD_SCHEDULE.length}</span></p>
          <p className="text-xs text-gray-500">已完成交稿</p>
        </div>
        <div className="bg-white rounded-xl border border-gray-200 px-3 sm:px-4 py-3 min-w-0">
          <p className="text-xs text-gray-500">素材逾期</p>
          <p className={`text-lg sm:text-xl font-bold mt-0.5 ${adOverdue.length > 0 ? 'text-red-500' : 'text-gray-500'}`}>
      ...
      {/* ── Tabs：手機可橫向捲動，不會擠掉任何分頁 ── */}
      <div className="px-4 sm:px-6 bg-white border-b flex overflow-x-auto shrink-0">
        {[['payments', '💳 付款進度'], ['events', '🏅 贊助賽事'], ['ads', '📁 廣告交稿']].map(([key, label]) => (
          <button key={key} onClick={() => setActiveTab(key)}
            className={`px-4 sm:px-5 py-3 min-h-[48px] text-sm font-medium border-b-2 transition-colors whitespace-nowrap ${
      ...
      <div className="flex-1 min-h-0 overflow-auto p-4 sm:p-6">
        ...
        {activeTab === 'payments' && (
          <div className="space-y-5 max-w-2xl">
            {[113, 114, 115].map(rocYear => {
              ...
              return (
                <div key={rocYear} className="bg-white rounded-xl border border-gray-200 overflow-hidden">
                  <div className="flex flex-wrap items-center justify-between gap-2 px-4 sm:px-5 py-3 bg-gray-50 border-b">
                  ...
                  <div className="divide-y divide-gray-100">
                    {yearPmts.map(payment => {
                      ...
                      return (
                        <div key={payment.id} className={`flex flex-wrap items-center justify-between gap-2 px-4 sm:px-5 py-3.5 ${isPaid ? 'bg-emerald-50/20' : ''}`}>
                          <div className="flex items-center gap-3 sm:gap-4 min-w-0">
                            {isManager ? (
                              <button onClick={() => togglePaid(payment.id, isPaid)}
                                aria-label={`${payment.label}${isPaid ? '：取消已付款標記' : '：標記為已付款'}`}
                                aria-pressed={isPaid}
                                className="w-11 h-11 -m-2.5 flex items-center justify-center flex-shrink-0 rounded-full hover:bg-gray-100">
                                <span className={`w-6 h-6 rounded-full border-2 flex items-center justify-center transition-colors ${isPaid ? 'bg-emerald-500 border-emerald-500 text-white' : 'border-gray-300'}`}>
                            ...
                            <div className="min-w-0">
                              <p className={`text-sm font-medium break-words ${isPaid ? 'text-gray-500 line-through' : 'text-gray-800'}`}>{payment.label}</p>
            ...
            <div className="bg-gray-800 rounded-xl px-4 sm:px-5 py-4 flex flex-wrap items-center justify-between gap-2 text-white">
              <div className="min-w-0">
                <p className="text-sm font-semibold">合約總計</p>
                <p className="text-xs text-gray-500 mt-0.5 break-words">民國 113–115 年 · 9 期款項</p>
              </div>
              <div className="text-right">
                <p className="text-lg sm:text-xl font-bold">{paidAmount} / 1,110 <span className="text-sm font-normal text-gray-500">萬</span></p>
            ...
            <div className="flex flex-wrap items-center gap-2 sm:gap-3 mb-4">
              <div className="flex rounded-lg border border-gray-200 overflow-hidden text-sm">
                {[2024, 2025, 2026].map(y => (
                  <button key={y} onClick={() => { setFilterYear(y); setEditingId(null); setShowAdd(false) }}
                    className={`px-3 sm:px-4 py-2 min-h-[40px] ${filterYear === y ? 'bg-blue-600 text-white' : 'bg-white text-gray-600 hover:bg-gray-50'}`}>
                    {y} 年
                  </button>
                ))}
              </div>
              ...
              {isManager && filterYear === 2026 && events.length === 0 && (
                <button onClick={importPreset} disabled={importing}
                  className="text-sm bg-emerald-600 text-white px-3 py-2 min-h-[40px] rounded-lg hover:bg-emerald-700 disabled:opacity-50">
                  {importing ? '匯入中…' : '⬇ 匯入 2026 賽程'}
                </button>
              )}
              {isManager && (
                <button onClick={() => { setShowAdd(v => !v); setEditingId(null); setAddForm(emptyForm) }}
                  className="sm:ml-auto text-sm bg-blue-600 text-white px-4 py-2 min-h-[40px] rounded-lg hover:bg-blue-700">
                  {showAdd ? '✕ 取消' : '+ 新增賽事'}
                </button>
              )}
            </div>
            {/* lg 以上才用 6 欄表格版型；lg 以下每一列改成堆疊卡片（欄位全部保留） */}
            <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
              <div className="hidden lg:grid px-4 py-2 border-b bg-gray-50 text-xs font-semibold text-gray-500 uppercase tracking-wide grid-cols-[1fr_72px_130px_110px_100px_72px]">
                ...
                return (
                  <div key={ev.id} className="border-b last:border-0">
                    {isManager && editingId === ev.id ? (
                      ...
                      <div className="flex flex-col gap-1.5 lg:grid lg:grid-cols-[1fr_72px_130px_110px_100px_72px] px-4 py-3 lg:items-center lg:gap-3 hover:bg-gray-50/60">
                        <div className="flex items-center gap-2 min-w-0">
                          <div className={`w-2 h-2 rounded-full flex-shrink-0 ${status.dot}`} />
                          <span className="text-sm font-medium text-gray-800 break-words lg:truncate">{ev.name}</span>
                        </div>
                        <div className="flex flex-wrap items-center gap-2 lg:block">
                          {ev.code && <span className={`text-xs font-semibold px-1.5 py-0.5 rounded border ${codeColor(ev.code)}`}>{ev.code}</span>}
                        </div>
                        ...
                        <div className="text-xs text-gray-500 break-words lg:truncate">{ev.note || '—'}</div>
                        <div>
                        ...
                        <div className="flex items-center gap-1 lg:justify-end">
                          {isManager && <button onClick={() => startEdit(ev)} className="text-xs text-blue-500 hover:text-blue-700 px-2 py-2 min-h-[36px] rounded hover:bg-blue-50">編輯</button>}
                          {isManager && <button onClick={() => setDeleteConfirm(ev.id)} className="text-xs text-red-400 hover:text-red-600 px-2 py-2 min-h-[36px] rounded hover:bg-red-50">刪除</button>}
                        </div>
                      </div>
                    )}
                  </div>
              ...
              <div className="flex rounded-lg border border-gray-200 overflow-hidden text-sm">
                {[['all','全部'], ['pending','待交稿'], ['done','已完成']].map(([key, label]) => (
                  <button key={key} onClick={() => setAdFilter(key)}
                    className={`px-3 sm:px-4 py-2 min-h-[40px] ${adFilter === key ? 'bg-blue-600 text-white' : 'bg-white text-gray-600 hover:bg-gray-50'}`}>
                    {label}
                  </button>
                ))}
              </div>
              ...
              {filteredGroups.map(group => {
                ...
                return (
                  <div key={group.dueDate} className="bg-white rounded-xl border border-gray-200 overflow-hidden">
                    ...
                    {/* Table header：lg 以上才是表格，以下改成每筆堆疊（欄位全部保留） */}
                    <div className="hidden lg:grid px-4 py-1.5 border-b bg-gray-50/60 text-xs font-medium text-gray-500 grid-cols-[28px_1fr_130px_150px_80px_130px]">
                    ...
                    <div className="divide-y divide-gray-100">
                      {group.items.map(item => {
                        ...
                        return (
                          <div key={item.id}
                            className={`flex flex-col gap-1.5 lg:grid lg:grid-cols-[28px_1fr_130px_150px_80px_130px] px-4 py-3 lg:items-center lg:gap-3 transition-colors ${isSubmitted ? 'bg-emerald-50/30' : ''}`}>

                            {/* Checkbox */}
                            {isManager ? (
                              <button onClick={() => toggleSubmitted(item.id, isSubmitted)}
                                title={isSubmitted ? '取消標記' : '標記已交稿'}
                                aria-label={`${item.item}${isSubmitted ? '：取消已交稿標記' : '：標記已交稿'}`}
                                aria-pressed={isSubmitted}
                                className="w-11 h-11 -m-3 flex items-center justify-center flex-shrink-0 rounded hover:bg-gray-100">
                                <span className={`w-5 h-5 rounded border-2 flex items-center justify-center transition-colors ${isSubmitted ? 'bg-emerald-500 border-emerald-500 text-white' : 'border-gray-300'}`}>
                            ...
                            <div className="min-w-0">
                              <div className="flex flex-wrap items-center gap-2">
                                <span className={`text-sm font-medium break-words ${isSubmitted ? 'text-gray-500 line-through' : 'text-gray-800'}`}>
                                  {item.item}
                                </span>
                            ...
                            {/* Size */}
                            <div className={`text-xs font-mono break-words ${isSubmitted ? 'text-gray-500' : 'text-gray-600'}`}>{item.size}</div>
                            ...
                            {/* Note */}
                            <div className={`text-xs break-words ${isSubmitted ? 'text-gray-500' : 'text-gray-500'}`}>
                              {item.note || '—'}
                            </div>
                          </div>
                        )
